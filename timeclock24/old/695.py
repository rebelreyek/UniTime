...
def app_main_loop(roster):
    ...
    while True:
        ...
        with roster_lock:
            user_found = False
            for lp in range(0, len(G_roster)):
                if user_id == G_roster[lp]['BarcodeID']:
                    user_found = True
                    break
            if user_found == False:
                continue
...
def app_post_loop(who, clock):
    ...
    while True:
        ...
        if len(list(G_events)) == 0 and len(list(G_roster)) != 0:
            ...
            who.place(x=5, y=5)
            clock.config(fg='darkgoldenrod', font='Arial 24 bold')
        ...
        if rosterreload == 600 or len(list(G_roster)) == 0:
            ...
            with roster_lock:
                try:
                    # pull roster in two steps in case gooogle rate limits
...
if __name__ == "__main__":
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name('client_secret_695.json', scope)
    ...
    who = Label(G_win, text="Who's here today:", fg="deepskyblue3", bg="black", font='Arial 32 bold', anchor='w')
    clock = Label(G_win, text="00:00:00", bg="black", font='Arial 24 bold', anchor='w')
    roster = Label(G_win, text=" ", fg="forestgreen", bg="black", font='Arial 18', anchor='w')
